...
#define I2C_BUS_AVAILABLE   (          2 )              // I2C Bus available in imx8mn-evk board
#define SLAVE_DEVICE_NAME   (   "OLED"   )              // Device and Driver Name
#define SSD1306_SLAVE_ADDR  (       0x3C )              // SSD1306 OLED Slave Address
 
static struct i2c_adapter *agnes_i2c_adapter     = NULL;  // I2C Adapter Structure
static struct i2c_client  *agnes_i2c_client_oled = NULL;  // I2C Cient Structure (In our case it is OLED)
...
static int I2C_Write(unsigned char *buf, unsigned int len)
{
    ...
    */ 
    int ret = i2c_master_send(agnes_i2c_client_oled, buf, len);
...
static int I2C_Read(unsigned char *out_buf, unsigned int len)
{
    ...
    ** ACK/NACK and Stop condtions will be handled internally.
    */ 
    int ret = i2c_master_recv(agnes_i2c_client_oled, out_buf, len);
...
*/
static int agnes_oled_probe(struct i2c_client *client,
...
*/
static int agnes_oled_remove(struct i2c_client *client)
{   
    //fill the OLED with this data
    SSD1306_Fill(0x00);
...
*/
static const struct i2c_device_id agnes_oled_id[] = {
        { SLAVE_DEVICE_NAME, 0 },
        { }
};
MODULE_DEVICE_TABLE(i2c, agnes_oled_id);
 
/*
** I2C driver Structure that has to be added to linux
*/
static struct i2c_driver agnes_oled_driver = {
        .driver = {
            .name   = SLAVE_DEVICE_NAME,
            .owner  = THIS_MODULE,
        },
        .probe          = agnes_oled_probe,
        .remove         = agnes_oled_remove,
        .id_table       = agnes_oled_id,
};
...
*/
static int __init agnes_driver_init(void)
{
    int ret = -1;
    agnes_i2c_adapter     = i2c_get_adapter(I2C_BUS_AVAILABLE);
    
    if( agnes_i2c_adapter != NULL )
    {
        agnes_i2c_client_oled = i2c_new_client_device(agnes_i2c_adapter, &oled_i2c_board_info);
        
        if( agnes_i2c_client_oled != NULL )
        {
            i2c_add_driver(&agnes_oled_driver);
            ret = 0;
        }
        
        i2c_put_adapter(agnes_i2c_adapter);
    }
...
*/
static void __exit agnes_driver_exit(void)
{
    i2c_unregister_device(agnes_i2c_client_oled);
    i2c_del_driver(&agnes_oled_driver);
    pr_info("Driver Removed!\n");
}
 
module_init(agnes_driver_init);
module_exit(agnes_driver_exit);
 
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Andre Ribeiro <andre.ribeiro.srs@gmail.com>");
